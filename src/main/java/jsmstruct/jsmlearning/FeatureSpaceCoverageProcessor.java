...
public class FeatureSpaceCoverageProcessor {
	...
	public Float calcDistance(String[] seed, String[] candidate)
			throws Exception {
		if (paramMap.isEmpty())
		...
		try {
			...
			if (latDiff > 1 || longDiff > 1)
				...
				score += latDiff.floatValue() / 100.0f + longDiff.floatValue()
						/ 100.0f;
		} catch (Exception e) {
			return 1000000f;
		}
		...
		for (int v = 0; v < seed[0].length; v++) {
			try {
				...
				if (seed[p3][v].equals("") || seed[p4][v].equals("")
					...
					continue;
				Double latDiff = Math.abs(Double.parseDouble(seed[p3][v])
						- Double.parseDouble(candidate[p3]));
				Double longDiff = Math.abs(Double.parseDouble(seed[p4][v])
						- Double.parseDouble(candidate[p4]));
				if (!(latDiff > 1 || longDiff > 1))
					currLatLongScore = latDiff.floatValue() / 100.0f
	...
	public Map<String, String> computeIntersection(String[] line1,
		...
		for (String attr : attributes) {
			...
			String v1 = line1[attrIndex].toLowerCase().replace("\"", "")
					.replace(",  ", ", ").replace(", ", ",");
			;
			String v2 = line2[attrIndex].toLowerCase().replace("\"", "")
					.replace(",  ", ", ").replace(", ", ",");
			;
			...
			if (valArr1Str == null || valArr2Str == null) { // we assume single
				...
				List<String> valList1 = new ArrayList<String>(
						Arrays.asList(valArr1));
				List<String> valList2 = new ArrayList<String>(
						Arrays.asList(valArr2));
...
//		System.out.println("ruleCoversCase attr_value " + attr_value);
//		System.out.println("ruleCoversCase line " + Arrays.toString(line));
		boolean soFarCovers = true;
		for (String attr : attributes) {
			int attrIndex = getIdForAttributeName(attr);
			String rule = attr_value.get(attr);
//			System.out.println("ruleCoversCase rule " + rule);
			if (rule == null)
				continue; // no constraint
			rule = rule.toLowerCase().replace("\"", "").replace(",  ", ",")
					.replace(", ", ",");
			String vCase = line[attrIndex].toLowerCase().replace("\"", "")
					.replace(",  ", ",").replace(", ", ",");
			...
			String valArrCaseStr = StringUtils
					.substringBetween(vCase, "{", "}");
			String valArrRuleStr = StringUtils.substringBetween(rule, "{", "}");
//			System.out.println("valArrCaseStr " + valArrCaseStr);
//			System.out.println("valArrRuleStr " + valArrRuleStr);
//			System.out.println("vCase " + vCase);
//			System.out.println("rule " + rule);
			if (valArrCaseStr == null || valArrRuleStr == null) { // we assume
				...
				List<String> valListCase = new ArrayList<String>(
						Arrays.asList(valArrCase));
				List<String> valListRule = new ArrayList<String>(
						Arrays.asList(valArrRule));
	...
	public boolean ruleCoversRule(Map<String, String> attr_value,
		...
		for (String attr : attributes) {
			...
			String valArrRuleBeingCoveredStr = StringUtils.substringBetween(
					vRuleBeingCovered, "{", "}");
			String valArrRuleStr = StringUtils.substringBetween(rule, "{", "}");
			if (valArrRuleBeingCoveredStr == null || valArrRuleStr == null) { // we
				...
				String[] valArrRuleBeingCovered = valArrRuleBeingCoveredStr
						.split(",");
				String[] valArrRule = valArrRuleStr.split(",");
				List<String> valListRuleBeingCovered = new ArrayList<String>(
						Arrays.asList(valArrRuleBeingCovered));
				List<String> valListRule = new ArrayList<String>(
						Arrays.asList(valArrRule));
	...
	@SuppressWarnings("unused")
	public Map<String, String> computeIntersection(Map<String, String> rule1,
			Map<String, String> rule2) {
		Map<String, String> attr_value = new HashMap<String, String>();
		for (String attr : attributes) {
			int attrIndex = getIdForAttributeName(attr);
	...
	public static void main(String[] args) {
		String[] attributes = new String[] { "w", "y", "g", "b", "f", "nf", "s",
				"ns", "r", "nr"};
		FeatureSpaceCoverageProcessor distProcessorPos = new FeatureSpaceCoverageProcessor();
		distProcessorPos.initParamMap(attributes, attributes);
		Map<String, String> rule = new HashMap<String, String>();
		rule.put("f","0");
		rule.put("w","0");
		rule.put("b","0");
		rule.put("nf","1");
		String[] example = {"1", "0", "0", "0", "0", "1", "0", "1", "1", "0"};
		
		System.out.println(distProcessorPos.ruleCoversCase(rule, example));
	}
}
